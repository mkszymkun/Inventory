...
class Graphics(tk.Frame):
    ...
    def login_label(self, text, row, col):
        tk.Label(self, text=text, font='Arial 10',
                 width=20, height=2, relief='groove').grid(row=row, column=col)

    def login_button(self, text, commands, row):
        tk.Button(self, text=text, font='Arial 10',
                  width=30, height=4, relief='groove',
    ...
    def menu_button(self, text, command, row):
        tk.Button(self, text=text, font='Arial 10',
                  width=50, height=6, relief='groove',
    ...
    def menu_header(self):
        tk.Label(self, text="MENU", font='Arial 15 bold',
                 width=30, height=5).grid(row=1, column=1)
    ...
    def button(self, text, commands, row):
        tk.Button(self, text=text, font='Arial 10', width=30, height=4,
                  relief='groove', command=commands).grid(
            row=row, column=0, columnspan=4)

    def location_label(self, key, row):
        tk.Label(self, text=key.upper(), width=50, height=2,
                 font='Arial 10 bold', relief='groove').grid(row=row, column=0)
    ...
    def location_name_button_colored(self, text, command, row):
        tk.Button(self, text=text, width=20, height=3, relief='groove',
                  bg='blue', command=command).grid(
            row=row, column=0, rowspan=2, sticky='w')
    ...
    def item_name_button_colored(self, k, v, reserved_and_available,
        ...
        tk.Button(self, text="{}  -  {} / {}  ({})".format(
            k, v, reserved_and_available, reserved), width=25, relief='groove',
                  bg='blue', command=command).grid(row=row, column=1,
                                                   columnspan=2)
    ...
    def reserved_item_name_button_colored(self, item, reserved, available,
                                          command, row):
        tk.Button(self, text="{}  -  {} szt. ({})".format(
            item, reserved, available), width=25, relief='groove', bg='blue',
                  command=command).grid(row=row, column=2, sticky='w')

    def reserved_item_name_button(self, item, reserved, available,
                                  command, row):
        tk.Button(self, text="{}  -  {} szt. ({})".format(
            item, reserved, available), width=25, relief='groove',
                  command=command).grid(row=row, column=2, sticky='w')

    def search_results_label(self, item):
        tk.Label(self, text="WYNIKI DLA \"{}\"".format(item),
                 font='Arial 15 bold', width=60, height=5,
    ...
    def reservation_label(self, user, reserved, row):
        tk.Label(self, text="{} - {}".format(user, reserved)).grid(
            row=row, column=3, columnspan=4)
